---
# SYSTEM-OWNED - do not edit. Generated by the DSL course sync, which clears and
# rewrites this whole collection on every run. Edit the source instead: the cohort's
# classroom-config/schedule.yml (dates, titles) or its org structure (what released).
type: lab
date: 2026-09-10T14:00:00
title: "Lab 6"
subtitle: "Lab 6"
unreleased: true
links: []
---
**Materials for lab 6 are not yet released** - they will appear in [`materials/labs/06_sequence-methods-and-time-series`](https://github.com/hertie-dsl-demo-f2026/materials/tree/main/labs) when they are.
